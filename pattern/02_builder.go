...
+ Изолирует сложный код сборки от бизнес логики

Минусы:
- Усложняет код программы из-за введения дополнительных типов
- Клиент может быть привязан к конкретным типам строителей, если в интерфейсе директора не будет метода получения результата.
*/

//пример использования паттерна
func builderPattern() {
	//создаем строителя который умеет создавать продукт (телефон сяоми)
	xiaomiBuilder := getBuilder("xiaomi")

	//чтобы не идти по шагам самим, используем тип директор, который знает как работать со строителем
